...
class DataHandler:
    ...
    def __init__(self, train_data, test_data, positive_classes, split_size, split_type, iteration, clean_stuff=None):
        ...
        if clean_stuff != None:
            print("NEED CLEAN!")
            self.cleaner = getattr(importlib.import_module(
                clean_stuff[1]), clean_stuff[2])()
    ...
    def read_data(self, data, train):
        ...
        for author in sorted(data):
            ...
            for book in sorted(books):
                ...
                # string_data = list(text_data.values())[0] # MY INSERT!!
                new_data[author][book] = self.preprocess_text(
                    text_data, author, book)
        return new_data
        ...
        for author, books in data.items():
            ...
            for book, texts in books.items():
                if level == "book":
                    ...
                    X.append(texts)
                    bookinfo.append(
                        [author, book, "book_split", positive_status])
                    y.append(author)
                elif level == "single":
                    for text_index, text in enumerate(texts):
                        X.append(text)
                        y.append(author)
                        bookinfo.append([author, book, "{}_{}".format(
                            text_index*self.split_size, (text_index+1)*self.split_size), positive_status])
                else:
                    raise NotImplementedError(
                        "Level {} not valid.".format(level))
    ...
    def cross_validation(self, level):
        ...
        if self.iteration < 0:
            for i in range(0, len(data)):
                X_train, y_train, train_info, X_test, y_test, test_info = self.extract_sets(
                    i, data, y, info)
                yield X_train, y_train, train_info, X_test, y_test, test_info, i+1, len(data)
        else:
            X_train, y_train, train_info, X_test, y_test, test_info = self.extract_sets(
                self.iteration, data, y, info)
    ...
    def true_data(self, level):
        train_data, train_y, train_info = self.data_to_list(
            level, self.train_data)
