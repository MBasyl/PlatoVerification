...
class SVDDModel:
    ...
    @staticmethod
    def vectorize_data(X_train, X_test, feature, ngram, max_features=3000):
        tfidf = TfidfVectorizer(strip_accents=None, lowercase=False,
                                analyzer=feature, ngram_range=ngram, max_features=max_features)
        # Training data
    ...
    @staticmethod
    def perform_svdd(df: pd.DataFrame, params: str, feature: str, ngram: tuple, bandwidth: float):
        if feature not in ['word', 'char']:
            raise ValueError(
                "Invalid value for 'feature'. Allowed values are 'word' or 'char'.")
        os.makedirs("outputs/SVDD", exist_ok=True)
        sys.stdout = open(f'outputs/SVDD/SVDD_log.txt', 'a')
        print("\n\n", params)
        testdata = df[df['title'].str.contains('test')]
        X_test = testdata['text']
        ytest = testdata['binary_label']
        test_names = testdata['title'].to_list()

        traindata = df[~df['title'].str.contains('test')]
        X_train = traindata['text']
        ytrain = traindata['binary_label']

        Xtrain, Xtest = SVDDModel.vectorize_data(
            X_train, X_test, feature=feature, ngram=ngram)
        clf = SVDD(kernel_type='rbf', bandwidth=bandwidth)
        # Train model
        ...
        for idx, label in enumerate(y_pred):
            status = "Misclassified" if label != ytest[idx] else "Correct"
            print(
                f"{status} - Predicted {label} for Author: {testdata['author'].iloc[idx]}, {testdata['title'].iloc[idx]}")
...
class SVDD(BaseEstimator, ClassifierMixin):
    ...
    def optimize_SVDD(self, X, y, c):
        ...
        borderx = SV_inds[np.where(
            (alfs[SV_inds] < ub[SV_inds]) & (alfs[SV_inds] > 1e-8))[0]]
    ...
    def fit(self, X, y):
        ...
        self.offs = 1 + \
            self.SV_alfs.T @ self.compute_kernel(self.SVx,
                                                 self.SVx) @ self.SV_alfs
    ...
    def predict(self, X):
        ...
        self.newout = np.vstack(
            [self.out, -1 * np.ones(self.m) * self.threshold])

        # Eq. 2.14: Indicator function
        I_out = np.greater_equal(
            self.newout[0, :], self.newout[1, :]).astype(int)
    ...
    def _plot_contour(self, X, y, document_names=None, lims=None):

        fig, ax = plt.subplots()
        s = 40
        ax.scatter(X[y == 1, 0], X[y == 1, 1], c='blueviolet', edgecolors='k',  # marker='+',
                   zorder=2, label='target')
        ax.scatter(X[y == -1, 0], X[y == -1, 1], c='gold',
                   edgecolors='k', zorder=2, label='outlier')
        if document_names:
        ...
        if self.SVx[0] in X:
            ax.scatter(X[self.SV_inds, 0], X[self.SV_inds, 1], facecolors='none',
                       edgecolors='gray', zorder=2, label='support vector')
        ...
        # Draw decision line
        ax.contour(X1, X2, Z, levels=(self.threshold,),
                   colors='red', linestyles='dashed', zorder=1)
