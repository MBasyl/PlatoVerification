...
def make_dataset(data_type: str) -> pd.DataFrame:
    ...
    authors, text_name, contents = utils.make_dataset_dictionary(
        folder=f'data/{data_type}Profiles', max_length=4300)
    dataset = {
        'author': authors,
        'title': text_name,
        'text': contents}
    ...
    df = insert_data(df, f"data/{data_type}/PsPla_The.txt")
    df = insert_data(df, f"data/{data_type}/PsPla_Min.txt")
    df = insert_data(df, f"data/{data_type}/PsPla_Hip.txt")
    df = insert_data(df, f"data/{data_type}/Disputed_VII.txt")
...
def split_Laws(laws_file, data_type, chunk_size):
    ...
    for i, chunk in enumerate(chunks):
        output_file_path = os.path.join(
            f"configure{data_type}laws", f"Pla_Law{i + 1}.txt")
...
def obfuscate(folder, p):
    for filename in os.listdir(folder):
        ...
        if author == 'Pla' and title in ['Law3', 'Law7', 'Law10', 'Law15', 'Law20']:
            ...
            lovers = open("configurePLAINlaws/PsPla_Lov.txt",
                          "r", encoding="utf-8")
            subs = lovers.read().split(". ")
            try:
...
# REMOVE: Law and Lov from folder
data_type = 'PLAIN'
...
laws_file = open(
    f"configure{data_type}laws/Pla_Law.txt", "r", encoding='utf-8').read()
...
for p in percentage:
    obfuscate(folder=f'configure{data_type}laws', p=p)

# DELETE PsPla_Lov and Laws used in obfuscation
df = pd.read_csv(f"{data_type}dataset_unobfusc.csv")
for file in glob.glob(f"configure{data_type}laws/*.txt"):
